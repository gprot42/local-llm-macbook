...
**Why these exact values:** in an A/B on an identical "build a Breakout game" task (see MODIFICATIONS.md), this recipe (thinking + `reasoning_effort: medium` + the card's thinking sampling + verbatim tool output) produced a structurally complete game and drove the model to *write its own headless test harness*, where the fast reasoning‑off default produced a visually broken one. The cost: ~30 min and ~7× the output tokens, and it still shipped one control bug — so it is the entry for a hard turn, not the everyday default. **Caveat:** that A/B changed several levers together (reasoning, sampling, prune), so it shows the *combination* helped on one task — not which lever, and not that this is a generally reliable agentic baseline.
