...
| `opencode.json` + `install-opencode-json.sh` | OpenCode provider fragment + installer (merges into `~/.config/opencode/opencode.json`, sets `model`/`small_model`, installs `agent.build.prompt`). Two model entries: `ternary-bonsai-2-27b` (default) and `ternary-bonsai-2-27b-think` (per-request thinking, 1024 budget). |
...
7. **Doom loop on a foreground dev server (build-agent prompt rules).** A "build a Mario Kart game" session wrote `index.html` + `mario-kart.js` (18.5 KB, `node --check` clean) in the first 20 minutes, then spent ~90 minutes in a loop found in OpenCode's session store (`~/.local/share/opencode/opencode.db`): `node -e "http.createServer(...)"` in the foreground with a 10 s tool timeout (killed — and the tool's hint says "retry with a larger timeout"), `curl localhost:8765` (dead, `=== 0`), start again — **148 server starts, 285 curls, 4 compactions**, no further writes. Alternating commands never trip OpenCode's `doom_loop` detector, and each compaction erased the memory of the failures. Fix: three rules in the build-agent prompt (no foreground servers — `nohup … &` or don't serve static files at all, verify with `node --check` and tell the user to open the file; never a third retry of a command that failed twice with the same result; stop once the files are written and checked, ending with what changed / how verified / how to run). `install-opencode-json.sh` now installs `agent.<name>.prompt` from the fragment (prompt only — an agent's temperature/permission are left alone). The prompt is model-agnostic because `agent.build` is global across OpenCode providers.

8. **A/B: our fast default vs. a thinking recipe (issue #183 follow-up).** After a commenter on [Bonsai-demo#183](https://github.com/PrismML-Eng/Bonsai-demo/issues/183) reported that per-request sampling + `reasoning_effort: medium` + verbatim tool output improved their results, we ran both on an identical "build a Breakout game" task (same anti-loop prompt, isolated `OPENCODE_CONFIG`, verified in a browser). **Arm A (our default:** reasoning off, non-thinking preset, prune on): 2.5 min, 3.5k output tokens, 4 tool calls — produced a game whose brick grid rendered off-screen and which showed "YOU WIN!" on load (structurally broken). **Arm B (thinking medium + budget 2048 + card thinking sampling + prune off):** ~30 min, 24.6k output tokens, 15 tool calls (8 edits) — produced a correct grid/score/lives/launch **and wrote its own headless frame-simulation test harness**, but shipped one control bug (`e.code === "KeyArrowLeft"` instead of `"ArrowLeft"`, so the paddle can't move). Conclusion: the combined thinking configuration helped a lot on this one task, but slow and still not one-shot-shippable. **Caveat (raised by the commenter):** the arms changed reasoning, sampling, *and* prune together, so this isolates the *combination*, not which lever mattered — and Arm B used the model card's thinking sampling (temp 1.0 / top-p 0.95), not the commenter's own temp 0.2 / top-p 0.9. A clean follow-up would hold task/sampling/prompt/tool-output fixed and toggle thinking alone, then `reasoning_content` replay alone. We tuned the `-think` OpenCode entry to this recipe at **budget 1024** (half the time/context of 2048) as a **selectable hard-turn mode, not a claimed isolated baseline**; the fast reasoning-off entry stays the default for long agentic builds.

9. **Lever-isolation factorial (issue #183 follow-up).** The item-8 A/B was confounded (reasoning+sampling+prune moved together). Clean version: same task/prompt/sampling (temp 1.0/top-p 0.95/presence 0)/prune=off, toggling one lever at a time — arm1 thinking off; arm2 thinking on + `reasoning_content` replay on; arm3 thinking on + replay off (`preserve_thinking: false`, verified to drop prior reasoning: 883→78 prompt tokens). Each browser-tested with **real `ArrowLeft`/`ArrowRight` events** (the check the model's own frame-sim missed in item 8). Result: **all three produced working games** (correct brick grid, arrows move the paddle, score/state). Thinking off: 2.5 min / 3.1k out tok / 3 tools; thinking on + replay on: 39 min / 36k / 32 (spent ~25 min on a self-written Node harness that kept erroring); thinking on + replay off: 12 min / 14k / 8. Conclusions: (a) **no lever produced a quality difference on this task** — the item-8 "thinking wins" result was run-to-run variance at temp 1.0, not the thinking lever; (b) **replay off is ~3× cheaper than replay on for the same outcome**, so the `-think` entry now sets `preserve_thinking: false`; (c) the fast reasoning-off default is vindicated (as good, far faster). Caveat: n=1 per cell; variance dominates, so these are bounds, not point estimates.
