...
| `2_start_llama.sh` | Serve on `:8089` — `llama-server -m <PQ2_0> --mmproj <mmproj> --alias ternary-bonsai-2-27b --jinja -ngl 999 -fa on -c 81920 -np 1 --cache-ram 24576 --reasoning off --temp 0.7 --top-p 0.8 --top-k 20 --min-p 0 --presence-penalty 1.5`. `status` / `stop` subcommands; `--think` / `--think-budget N` switch to the thinking preset. |
...
- **Text + image**, native tool calling (BFCL v3 ~74), **thinking model** (`xhigh` effort by default; served with reasoning off, see Fixes).
...
5. **Reasoning effort.** The card says the 27B defaults to `xhigh` effort (`low` is unsupported); PrismML's own answer to slow/exhausted replies is `--reasoning-budget N` or `BONSAI_THINKING=0`. Added `--think-budget N` (≈ their "Medium" at 2048) beside `--think`; the deprecated `--chat-template-kwargs '{"enable_thinking": false}'` was dropped (`--reasoning off` alone verified: `reasoning_content` empty, no `<think>` leak). Thinking mode also gets `--reasoning-preserve` (the template supports it; the `@ai-sdk/openai-compatible` provider does send `reasoning_content` back).
...
- **Output exhausted while reasoning** ("produced no actionable output", e.g. the mario.html write never landed): the thinking model spent its whole budget reasoning. Default the server to **`--reasoning off`** (initially paired with `--chat-template-kwargs '{"enable_thinking": false}'`, since dropped as deprecated — `--reasoning off` alone suffices) via a `--think`/`BONSAI_THINK` toggle so it emits tool calls directly; set OpenCode `reasoning: false`. Re-enable thinking with `./2_start_llama.sh --think` or cap it with `--think-budget N`.
