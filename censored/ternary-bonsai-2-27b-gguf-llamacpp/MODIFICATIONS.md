...
| `2_start_llama.sh` | Serve on `:8089` — `llama-server -m <PQ2_0> --mmproj <mmproj> --alias ternary-bonsai-2-27b --jinja -ngl 999 -fa on -c 81920 -np 1 --cache-ram 24576 --reasoning on --reasoning-preserve --reasoning-budget 2048 --temp 1.0 --top-p 0.95 --top-k 20 --min-p 0 --presence-penalty 0`. `status` / `stop` subcommands; `--think-budget N` (`-1` unlimited) / `--no-think` (non-thinking preset `0.7/0.8/20/presence 1.5`). |
...
| `opencode.json` + `install-opencode-json.sh` | OpenCode provider fragment + installer (merges into `~/.config/opencode/opencode.json`, sets `model`/`small_model`). |
...
- **Text + image**, native tool calling (BFCL v3 ~74), **thinking model** (`xhigh` effort by default; served with reasoning on under a 2048-token budget, see below).
...
5. **Reasoning on by default, budgeted (user preference).** The card says the 27B defaults to `xhigh` effort (`low` is unsupported); PrismML's own answer to slow/exhausted replies is `--reasoning-budget N`. Default is now `--reasoning on --reasoning-budget 2048 --reasoning-preserve` (≈ their "Medium"); `--think-budget N` / `--no-think` adjust it, and the deprecated `--chat-template-kwargs '{"enable_thinking": false}'` was dropped (`--reasoning off` alone verified). OpenCode model set `"reasoning": true`. Verified live: a prompt engineered to think indefinitely was cut mid-thought at the budget and still produced the answer; tool calls come out after reasoning (`finish=tool_calls`); and a tool-loop turn that sends `reasoning_content` back re-processed only 32 tokens (the `@ai-sdk/openai-compatible` provider does send it, and `--reasoning-preserve` renders it, so the prefix cache holds).
...
- **Output exhausted while reasoning** ("produced no actionable output", e.g. the mario.html write never landed): the thinking model spent its whole budget reasoning. First fix was `--reasoning off` (paired with the since-deprecated `--chat-template-kwargs '{"enable_thinking": false}'`); superseded by the budgeted default above (`--reasoning-budget 2048`), which keeps thinking but guarantees room for the tool call. `--no-think` still gives the direct mode.
