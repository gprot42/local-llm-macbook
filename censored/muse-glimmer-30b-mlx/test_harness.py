#!/usr/bin/env python3
"""Standalone harness resilience tests for Muse Glimmer 30B + mlx-vlm.

Runs *outside* Kilo against the OpenAI-compatible mlx_vlm API (default :8087).

Usage:
  ./2_start_mlx.sh
  python3 test_harness.py
  python3 test_harness.py --base http://127.0.0.1:8087 --strict
  python3 test_harness.py --gate
  python3 test_harness.py --quick
  python3 test_harness.py --model muse-glimmer-30b-mlx

Muse Glimmer always thinks. Short max_tokens can blank both content and
reasoning_content. Tests use Reasoning strength: low and generous max_tokens.
...
DEFAULT_MODEL = "muse-glimmer-30b-mlx"

LOW_REASONING = {"role": "system", "content": "Reasoning strength: low."}
...
def _with_low_reasoning(messages: list[dict]) -> list[dict]:
    if messages and messages[0].get("role") == "system":
        return messages
    return [LOW_REASONING, *messages]
...
def run_gate_tests(base: str, model: str, report: Report) -> bool:
    ...
    try:
        ...
        text = _visible_text(msg).strip()
        report.check(
            "gate: short non-tool chat",
            code == 200 and bool(text),
            f"text={text[:60]!r} {elapsed:.2f}s",
        )
    except Exception as e:
    ...
    try:
        ...
        msg = _msg(data) if isinstance(data, dict) else {}
        content = _visible_text(msg).lower()
        hard_ok = code == 200
        soft_ok = hard_ok and (
            "done" in content
            or "step1" in content
        ...
        report.check(
            ...
            soft_ok if hard_ok else False,
            f"content={_visible_text(msg)[:50]!r} tools={_tool_names(msg)} {elapsed:.2f}s",
            soft=hard_ok and not soft_ok,
        )
    except Exception as e:
...
def run_live_tests(
    ...
    *,
    ...
    try:
        ...
        report.check("live: GET /v1/models", ok, f"status={code} {elapsed:.2f}s")
        if not ok:
            return False
    except Exception as e:
    ...
    try:
        ...
        report.check(
            ...
            code == 200 and bool(text),
            f"text={text[:80]!r} finish={_finish(data)!r} {elapsed:.2f}s",
        )
    except Exception as e:
                ...
                [
                    {
                        "role": "user",
                        "content": (
                            "Call the bash tool with command exactly: echo harness_ok"
                        ),
                    }
                ]
            ),
    ...
    try:
        ...
        report.check(
            ...
            code == 200 and bool(_visible_text(msg).strip()),
            f"chars={len(_visible_text(msg))} {elapsed:.2f}s",
        )
    except Exception as e:
    ...
    try:
        code, data, elapsed = _chat(
            ...
            _with_low_reasoning(
                [
                    {"role": "user", "content": "Run bash: echo x. Then continue."},
                    {
                        "role": "assistant",
                        "content": None,
                        "tool_calls": [
    ...
    try:
        ...
        msg = _msg(data) if isinstance(data, dict) else {}
        names = _tool_names(msg)
        content = _visible_text(msg)
        hard_ok = code == 200
        soft_ok = hard_ok and (
            "bash" in names or "step2" in content.lower() or bool(content.strip())
        )
        report.check(
        ...
        report.check("live: multi-step continues after tool result", False, str(e))
    ...
    try:
        ...
        def one(i: int) -> tuple[int, float]:
            ...
            c, _, _ = _chat(
                base,
                _with_low_reasoning([{"role": "user", "content": f"Say {i}"}]),
                max_tokens=64,
                tools=None,
                model=model,
                timeout=180,
    ...
    try:
        code, data, elapsed = _http_json(
            ...
            "POST",
            "/v1/chat/completions",
            {"model": model, "messages": [], "max_tokens": 8},
            timeout=30,
        )
        report.check(
...
def main(argv: list[str] | None = None) -> int:
    ap = argparse.ArgumentParser(description="Muse Glimmer 30B mlx-vlm harness smoke tests")
    ap.add_argument("--base", default=DEFAULT_BASE, help="API base URL")
    ap.add_argument(
        "--model",
        default=DEFAULT_MODEL,
        help="Model id (auto-resolved from /v1/models if missing)",
    )
    ap.add_argument("--gate", action="store_true", help="Post-start gate only")
    ap.add_argument("--quick", action="store_true", help="Skip slower live tests")
    ap.add_argument(
        "--strict",
        action="store_true",
    ...
    print("Muse Glimmer 30B mlx-vlm harness tests")
    print(
        f"  base={args.base}  model={args.model}  strict={args.strict}  "
        f"gate={args.gate} quick={args.quick}"
    )
    print("  Note: this is NOT a full Kilo emulator. Thinking is always on.")
    ...
    if args.gate:
        ...
        connected = run_live_tests(args.base, model, report, quick=args.quick)
