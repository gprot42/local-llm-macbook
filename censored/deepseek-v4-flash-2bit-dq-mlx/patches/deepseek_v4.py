...
class DeepseekV4MoE(nn.Module):
    def __init__(self, config: ModelArgs, layer_idx: int):
        ...
        # Use unfused SwitchGLU: 2bit-DQ checkpoints quantize gate_proj at
        # group_size=32 and up_proj at group_size=64, so sanitize cannot fuse
        # them into FusedSwitchGLU (scale/bias shapes disagree).
...
class Model(nn.Module):
    ...
    def sanitize(self, weights: Dict[str, mx.array]) -> Dict[str, mx.array]:
        ...
        # Do not fuse gate/up: DeepseekV4MoE uses SwitchGLU (unfused).
        # 2bit-DQ also uses different group_size for gate vs up on most layers.
