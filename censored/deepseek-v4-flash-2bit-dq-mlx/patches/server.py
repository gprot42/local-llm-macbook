...
class ModelProvider:
    ...
    def _load(self, model_path, adapter_path=None, draft_model_path=None):
        ...
        # DeepSeek-V4 + BatchGenerator causes Metal GPU timeouts on first
        # completion (standalone stream_generate is fine). Force sequential.
        model_type = None
        try:
            model_type = getattr(model, "model_type", None) or getattr(
                getattr(model, "args", None), "model_type", None
            )
            if model_type is None and hasattr(model, "config"):
                model_type = getattr(model.config, "model_type", None)
        except Exception:
            model_type = None
        if model_type == "deepseek_v4" or "deepseek_v4" in str(model_path).lower() or "deepseek-v4" in str(model_path).lower():
            logging.info("Disabling BatchGenerator for DeepSeek-V4 (sequential stream_generate)")
            is_batchable = False
...
class ResponseGenerator:
    def __init__(self, model_provider: ModelProvider, prompt_cache: LRUPromptCache):
        ...
        self._stop = False
        # Do not accept HTTP until the default model has finished loading.
        # Concurrent requests during a ~100GB Metal load cause GPU timeouts.
    ...
    def _generate(self):
        ...
        try:
            ...
            logging.info("Default model loaded — accepting connections.")
        except Exception:
...
def run(
    ...
    if group.rank() == 0:
        logging.info(
            "Waiting for model to finish loading before opening the HTTP port..."
        )
        response_generator._model_ready.wait()
