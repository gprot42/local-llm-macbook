...
Apple Silicon stacks for **coding agents** (Kilo Code), chat, and experiments. OpenAI-compatible APIs; mostly MLX-native (`mlx_lm`, `vllm-mlx`, `mtplx`). Native Metal **ds4** for DeepSeek V4 Flash GGUF. Two stacks use Ollama + GGUF (Ornith 1.0, GLM).

Status: 🟢 working · 🟡 partial / flaky (e.g. Kilo filters some prompts). 🟡 stacks are **not** in the global [`kilo.json`](kilo.json) catalog.
...
| [`uncensored/qwen3-8-27b-obliterated-mtplx/`](uncensored/qwen3-8-27b-obliterated-mtplx/) | `mtplx-qwen38-obl/qwen3.8-27b-obliterated-mtplx` | 🟢 Uncensored Qwen 3.8 27B (OBLITERATUS, mtplx MTP) | Text | `:8768/v1` (proxy) |
...
| Uncensored Qwen 3.8 27B (mtplx MTP) | 🟢 **Qwen 3.8 OBLITERATED** | Need aligned refusals; co-load aligned 3.8 on `:8766` on tight RAM |
...
**DeepSeek sampling:** official defaults are `temperature=1.0`, `top_p=1.0` (see stack `kilo.json`). Qwen coding uses `0.6 / 0.95 / top_k 20`. Qwen 3.8 OBLITERATED uses **temp 0** (greedy). Muse Glimmer uses `1.0 / 0.95 / top_k 64` and always thinks (`Reasoning strength: low|medium|high|xhigh`).
...
| 🟢 | [`uncensored/qwen3-8-27b-obliterated-mtplx/`](uncensored/qwen3-8-27b-obliterated-mtplx/) | `http://localhost:8768/v1` | `mtplx-qwen38-obl/qwen3.8-27b-obliterated-mtplx` |
...
- [qwen3-8-27b-obliterated-mtplx/README.md](uncensored/qwen3-8-27b-obliterated-mtplx/README.md) — Qwen3.8-27B OBLITERATED V3 (mtplx MTP; local mlx convert from bf16)  
