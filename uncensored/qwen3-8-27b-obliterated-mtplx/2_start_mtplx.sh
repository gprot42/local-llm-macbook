...
#
# mtplx serves at http://localhost:8767/v1  (OpenAI-compatible)
# Port 8767 avoids clashing with aligned Qwen3.8 mtplx on :8766
# and Qwen3.6 mtplx on :8765.
#
# Options:
#   --port  PORT    Override port (default: 8767)
...
if [[ "${HF_MODEL}" == "${HF_REPO}" || "${HF_MODEL}" == "OBLITERATUS/Qwen3.8-27B-OBLITERATED" ]]; then
    ...
    echo "  That pack includes GGUF + bf16 (~270 GB). Use the local mlx subfolder:"
...
echo ""
echo "  Kilo Code:  model mtplx-qwen38-obl/qwen3.8-27b-obliterated-mtplx  (see kilo.json)"
