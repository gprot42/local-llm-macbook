...
class ChatCompletionRequest(BaseModel):
    ...
    # OpenAI-compatible token-level logit bias.  Keys are token IDs (as
    # strings, per OpenAI spec) and values are additive bias in [-100, 100].
    # -100 effectively bans a token.  Wired through to
    # `mlx_lm.sample_utils.make_logits_processors(logit_bias=...)`.  Added
    # by the local proxy patch to let the Kilo/Continue steering layer
    # suppress Harmony control tokens (e.g. `<|channel>`, `<|think|>`) that
    # the gemma-4-heretic finetune occasionally emits as a "channel" prefix
    # before a tool call -- those leaks break the upstream tool parser.
