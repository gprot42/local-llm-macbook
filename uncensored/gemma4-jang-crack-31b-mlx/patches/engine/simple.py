...
def _bind_worker_generation_streams(
    *,
    ...
    if clear_cache:
        try:
            ...
            logger.debug("MLX synchronize before stream rebind failed", exc_info=True)
        try:
            mx.clear_cache()
        except Exception:
            logger.debug("MLX clear_cache before stream rebind failed", exc_info=True)
    ...
    try:
        ...
        logger.debug("MLX set_default_stream during stream rebind failed", exc_info=True)
    ...
    if clear_cache:
        try:
            ...
            logger.debug("MLX clear_cache after stream rebind failed", exc_info=True)
...
class SimpleEngine(BaseEngine):
    ...
    async def chat(
        ...
        **kwargs,
        ...
        if self._is_mllm:
            ...
            async with self._generation_lock:
                _bind_worker_generation_streams(thread_local=True, clear_cache=True)
                try:
                    ...
                    try:
                        ...
                        logger.debug(
                            "MLX cache cleanup after media chat failed",
                            exc_info=True,
                        )
            ...
            async with self._generation_lock:
                ...
                try:
                    ...
                    try:
                        ...
                        logger.debug(
                            "MLX cache cleanup after media stream_chat failed",
                            exc_info=True,
                        )
            return
    ...
    async def _stream_generate_text(
        ...
        **kwargs,
        ...
        # OpenAI-compatible per-token logit bias.  Accepts either {int: float}
        # (the mlx_lm-native form) or {str: float} (the OpenAI-wire form);
        # we coerce string keys to int below.  Added by local proxy patch.
        logit_bias_raw = kwargs.pop("logit_bias", None)
        logit_bias: Optional[Dict[int, float]] = None
        if logit_bias_raw:
            try:
                logit_bias = {int(k): float(v) for k, v in logit_bias_raw.items()}
            except (TypeError, ValueError):
                logger.warning(
                    "ignoring malformed logit_bias=%r (expected {token_id: bias})",
                    logit_bias_raw,
                )
                logit_bias = None
        ...
        if has_system and self._text_model is not None:
            ...
            # System section ends where the first non-system turn begins.
            # Qwen/ChatML: <|im_start|>user; Gemma 4: <|turn>user / <|turn>model.
            system_prefix_end = -1
            for marker in (
                "<|im_start|>user\n",
                "<|im_start|>assistant\n",
        ...
        if full_token_count == 0:
            try:
                ...
                full_token_count = len(
                    tokenizer.encode(full_prompt, add_special_tokens=add_special)
                )
            except Exception:
                pass

        logger.info(
            "Text route: prompt ~%d tokens (%d chars), max_tokens=%d, "
            "system_kv=%s, tools=%s",
            full_token_count,
            len(full_prompt),
            max_tokens,
            "hit" if cache_hit else ("miss" if system_token_count else "n/a"),
            bool(tools),
        )
        ...
        def _run_all():
            ...
            _prefill_t0 = _time.monotonic()
            if isinstance(prompt_to_send, str):
                _prefill_n = full_token_count or "?"
            elif hasattr(prompt_to_send, "size"):
                _prefill_n = int(prompt_to_send.size)
            else:
                _prefill_n = "?"
            logger.info(
                "Text route: starting mlx_lm prefill+decode (%s prompt tokens, "
                "prefill_step_size=%d)",
                _prefill_n,
                ...
                for resp in mlx_stream_generate(
                    ...
                    **gen_kwargs,
                    ...
                    if not _first_token_logged:
                        logger.info(
                            "Text route: first token after %.1fs prefill+decode",
                            _time.monotonic() - _prefill_t0,
                        )
        ...
        token_count = 0
        finished = False
        empty_newtext_streak = 0
        _EMPTY_NEWTEXT_STREAK_LIMIT = 100
        try:
            while True:
                ...
                if not (new_text and str(new_text).strip()):
                    empty_newtext_streak += 1
                    if empty_newtext_streak >= _EMPTY_NEWTEXT_STREAK_LIMIT:
                        logger.warning(
                            "Text route: %d consecutive empty new_text tokens at "
                            "token %d — stopping (mlx empty-token loop)",
                            empty_newtext_streak,
                            token_count,
                        )
                        finished = True
                        yield GenerationOutput(
