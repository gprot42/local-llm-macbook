...
def _prepare_chat_completion_invocation(
    ...
    # OpenAI-compatible logit_bias forwarding.  Added by local proxy patch.
    # Keys are token IDs (as strings per OpenAI spec), values in [-100, 100];
    # -100 effectively bans a token.  Used by the gemma4_mlx_kilo_proxy proxy to
    # suppress Harmony control tokens that the gemma-4-heretic finetune
    # otherwise emits before tool calls.
    if request.logit_bias:
        chat_kwargs["logit_bias"] = request.logit_bias
...
async def stream_chat_completion(
    ...
    **kwargs,
    ...
    # Gemma 4 + tools can enter an mlx empty-token loop until max_tokens
    # (Kilo sends max_tokens=16384). Without the proxy, abort here.
    _EMPTY_OUTPUT_STREAK_LIMIT = 100
    ...
    try:
        ...
        async for output in engine.stream_chat(messages=messages, **kwargs):
            ...
            if not output.finished:
                if not (delta_text and delta_text.strip()):
                    empty_output_streak += 1
                    if empty_output_streak >= _EMPTY_OUTPUT_STREAK_LIMIT:
                        logger.warning(
                            "[empty-delta-abort] %d consecutive engine outputs "
                            "with empty new_text (completion_tokens=%d) — "
                            "terminating stream",
                            empty_output_streak,
                            completion_tokens,
                        )
        ...
        if stream_aborted:
            abort_chunk = ChatCompletionChunk(
                id=response_id,
                model=_model_name,
                choices=[
                    ChatCompletionChunkChoice(
