#!/usr/bin/env bash
# =============================================================================
# 2_start_ollama.sh — Serve GLM-4.7-Flash Heretic via Ollama
#
# OpenAI-compatible API: http://127.0.0.1:18083/v1
#   (OpenCode/Kilo → proxy → Ollama :11434)
# Use 127.0.0.1 — not localhost (macOS resolves localhost to ::1; proxy is IPv4).
#
# Port 18083 so this can run alongside Ornith (18082) on the same machine.
#
# Requires:
#   brew install ollama
...
CONFIG_FILE="$SCRIPT_DIR/.glm47_config"
...
if [[ -n "${QUANT_OVERRIDE}" ]]; then
    case "${QUANT_OVERRIDE}" in
        q4) GGUF_FILE="${PREFIX}-Q4_K_M.gguf"; MODEL_ID="glm-4.7-flash-heretic-q4" ;;
        q5) GGUF_FILE="${PREFIX}-Q5_K_M.gguf"; MODEL_ID="glm-4.7-flash-heretic-q5" ;;
        q6) GGUF_FILE="${PREFIX}-Q6_K.gguf";   MODEL_ID="glm-4.7-flash-heretic-q6" ;;
        q8) GGUF_FILE="${PREFIX}-Q8_0.gguf";   MODEL_ID="glm-4.7-flash-heretic-q8" ;;
...
if [[ "$DO_STATUS" == true ]]; then
    echo "=== GLM-4.7-Flash Heretic status ==="
    if ollama_api_ok; then
...
if [[ "$USE_PROXY" == true ]]; then
    PROXY_SCRIPT="$SCRIPT_DIR/openai_proxy.py"
    ...
    echo ""
    echo "=== GLM-4.7-Flash Heretic — Ollama + OpenAI proxy ==="
...
echo ""
echo "=== GLM-4.7-Flash Heretic — Ollama direct (no proxy) ==="
echo "→ Model:     $MODEL_PATH"
echo "→ Ollama:    ${MODEL_ID} @ http://127.0.0.1:${OLLAMA_PORT}"
