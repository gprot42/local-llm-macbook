...
This proxy sits in front of mtplx and:
  ...
  6. Capability middleware (AutoSaddler-style, scoped — not always-on):
  ...
  7. Auto-fix tool calls (same as Ornith): bash-sanitize unclosed quotes /
     pipes, remap missing files to parent ls, restore leading '/', collapse
     duplicated workspace prefixes; FileNotFound counts as empty recovery
...
def _sanitize_bash_command(command: str, messages: list[dict] | None = None) -> str:
    ...
    if paths and (
        _UNSAFE_BASH_RE.search(cmd)
        or cmd.startswith("echo ")
        or cmd.startswith(("python", "find ", "sed ", "awk ", "for "))
    ):
        return _safe_head_command(paths, messages)
    if paths and messages:
        ...
        if known and not any(
            ...
            return _safe_head_command(paths, messages)
    if not _UNSAFE_BASH_RE.search(cmd) and cmd.startswith(_SAFE_BASH_STARTS):
        return cmd
    for piece in re.split(r"[\n;|&]+", cmd):
        s = piece.strip()
        if not s or _unmatched_quotes(s):
            continue
        if s.startswith(("echo ", "python", "find ", "sed ", "awk ", "for ")):
            continue
        if _is_pathless_head(s):
            continue
        if s.startswith(_SAFE_BASH_STARTS) and not _UNSAFE_BASH_RE.search(s):
            return s
    if paths:
        return _safe_head_command(paths, messages)
            ...
            if isinstance(data, dict):
                ...
                if path.rstrip("/").endswith("/chat/completions") or path.rstrip(
                    ...
                    log.info(
                        ...
                        "empty_rec=%s fake=%s prose=%s after_tool=%s trunc=%s",
            ...
            if stream or "text/event-stream" in content_type:
                ...
                while True:
                    ...
                    if not chunk:
                        break
                    chunks.append(chunk)
                payload = _repair_sse_payload(b"".join(chunks), steer_messages)
...
def _self_check() -> None:
    ...
    assert "autohunt\\\\" not in fixed
    assert fixed.startswith(("ls ", "head "))
    miss = (
